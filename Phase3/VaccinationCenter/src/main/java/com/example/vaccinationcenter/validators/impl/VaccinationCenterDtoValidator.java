...
public class VaccinationCenterDtoValidator implements ConstraintValidator<VaccinationCenterValidator, VaccinationCenterDto> {
	...
	private static class PostRequestValidator implements RequestDtoValidator<VaccinationCenterDto> {
		...
		public boolean validate(VaccinationCenterDto center, ConstraintValidatorContext context) {
			...
			if (center == null) { 
				...
				context.buildConstraintViolationWithTemplate("Center Name Should Not Be Null").addPropertyNode("center") .addConstraintViolation(); 
				return flag; 
				} 
			if (!StringUtils.hasText(center.getName())) {
				context.disableDefaultConstraintViolation();
				context.buildConstraintViolationWithTemplate("Center Name Required").addPropertyNode("name") .addConstraintViolation(); 
				flag = false;
				}
			...
			Predicate<VaccinationCenterDto> validateNameLength = (VaccinationCenterDto center1) -> {
				...
				if (validateNameLength.negate().test(center)) { 
					context.disableDefaultConstraintViolation(); 
					context. buildConstraintViolationWithTemplate("Center Name Should be between 4 and 60 characters").addPropertyNode("name") .addConstraintViolation(); 
					flag = false; 
					}
				
				if (!StringUtils.hasText(center.getCity())) {
					context.disableDefaultConstraintViolation(); 
					context.buildConstraintViolationWithTemplate("Center City Required").addPropertyNode("city") .addConstraintViolation(); 
					flag = false; 
					} 
				...
				Predicate<VaccinationCenterDto> validateAddressLength = (VaccinationCenterDto center1) -> {
					...
					if (validateAddressLength.negate().test(center)) { 
						context.disableDefaultConstraintViolation(); 
						context.buildConstraintViolationWithTemplate("Center City Should be between 15 and 100 characters").addPropertyNode("city") .addConstraintViolation();
						flag = false; 
						} 
					return flag;
					}
	...
	private static class PutRequestValidator extends PostRequestValidator { 
		...
		public boolean validate(VaccinationCenterDto dto, ConstraintValidatorContext context) { 
			...
			if (dto.getId() < 1) { flag = false; 
			context.disableDefaultConstraintViolation(); 
			context.buildConstraintViolationWithTemplate("center Id should be greater than 0") .addPropertyNode("id") .addConstraintViolation();
			}
			return flag;
			}
		}
