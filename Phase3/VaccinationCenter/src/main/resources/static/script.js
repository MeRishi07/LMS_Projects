...
function citizenListByUrl(url, deleteAction, tableId) { 
	let tableRow = {
		 citizensTable: function (citizen) {
			  ...
			  <button id="${citizen.id}" onclick="${deleteAction.name}(event)">Delete</button> </td>`; 
			}, 
			
			centerCitizensTable: function (citizen) { 
				return `<td>${citizen.id}</td> 
				<td>${citizen.name}</td> 
				<td> <button id="${citizen.id}" onclick="_citizenView(event)">View</button> </td>`; 
			}, 
		}; 
		...
		if (tableId) { 
			...
			function bindCitizenWithCallback(id, callback) { 
				...
				.then(function (data) { 
					...
					function _layoutAppendCenters(centers, citizen) { 
						...
						centerOptions.forEach(function (option) { 
							...
							optionElement.value = option.id; optionElement.text = option.id + " | " + option.name + " | " + option.address; 
				...
				function _layoutEditForm(citizen, centers) { 
					...
					document.getElementById("vaccinationStatus").innerHTML = citizen.vaccinationStatus; 
				...
				function centerFormAdd(event) { 
					...
					const centerCity = centerCitySelect.options[centerCitySelect.selectedIndex].text; 
			...
			function vaccinationCenters() { 
				...
				fetch(getBaseUrl() + "/vaccinationcenter", { 
					...
					.then(function (response) { 
						...
						var tableBody = document .getElementById("centerTable") .querySelector("tbody"); 
						centers.forEach(function (center) { var row = document.createElement("tr"); 
						row.innerHTML = ` 
						               <td>${center.id}</td> 
						               <td>${center.name}</td> 
			...
			function saveCitizen(event) { 
				...
				const vaccinationCountSelectValue = vaccinationCountSelect.options[vaccinationCountSelect.selectedIndex].value; 
			...
			.then((data) => { 
				...
				<div class="alert alert-success" role="alert"> Citizen Saved </div> `; 
				setTimeout(() => { 
					let messageElement = document.getElementById("errorList"); 
					messageElement.innerHTML = ""; 
					}, 2000); 
